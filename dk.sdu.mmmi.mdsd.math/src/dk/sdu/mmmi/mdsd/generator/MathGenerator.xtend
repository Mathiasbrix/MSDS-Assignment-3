...
class MathGenerator extends AbstractGenerator {
	...
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		for (exp: resource.allContents.toIterable.filter(MathExp)) {
			fsa.generateFile("math_expression/"+ exp.name + ".java", exp.compile)
		}
	...
	def static dispatch String computeExpression(Paranthesis exp) {
		"(" + exp.p.computeExpression +")"
	}
	...
	def static dispatch String computeExpression(FunctionCall exp) {
		...
		if(exp.args.size>0){
			for(arg: exp.args)
				args += arg.computeExpression+","
				exp +"("+ args.substring(0, args.length() - 1) +")"
		}else
			exp + "()"
	}
